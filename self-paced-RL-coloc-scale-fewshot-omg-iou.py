...
def init_dataloader():
    ...
    test_batchsampler = init_sampler(args.classes_per_set, args.samples_per_class*2,
                                     testset.y[:, -1], args.iterations, 'val')
...
def init_model():
    # using ae
    net = Net_ae(pooling_size=args.pooling_size,
...
def feed_data(model, agent, data_loader, is_train, upload=True):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        # rois
        actor = Actor(target_x, target_y, min_box_side=28, sign=args.sign)
        ...
        with torch.no_grad():
            target_embed = model(target_x, rois)
        ...
        for t in range(args.seq_len):
            ...
            with torch.no_grad():
                target_embed = model.get_roi_embedding(rois)
        ...
        q, score, critic_loss, adv = get_policy_loss(rewards_all, batch_size, gamma=args.gamma,

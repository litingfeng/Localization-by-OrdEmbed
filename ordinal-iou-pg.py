"""
Use ordinal embeddin+ IoU reward + policy gradient
...
def feed_data(model, agent, data_loader, is_train, upload=True):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        for t in range(args.seq_len):
            if args.rnn == 1:
                h_t, logits, actions, _ = agent(state, h_t_prev=h_t)
            else:
                logits, actions, _ = agent(state)
        ...
        if args.sign == 1:
            rewards_all = torch.sign(rewards_all)

"""
joint train by using agent trajectory
...
from models.mnist_scale_model import Agent_ae, Net_ae_proj
...
def init_model():
    ...
    if args.freeze == 1:
        for name, value in net.named_parameters():
            if 'encoder' in name: # only freeze encoder for ae with projection
                print('freezing ', name)
...
def feed_data(model, agent, data_loader, is_train, upload=True):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        for t in range(args.seq_len):
            if args.rnn == 1:
                h_t, logits, actions = agent(embed.detach(), h_t_prev=h_t)
            else:
        ...
        _, feature = model(data, rois.detach())
        feature = feature.view(batch_size, 2, -1)  # (bs, 2, 1024)

        # compute loss order
        inds = torch.max(iou, 1).indices
        p = feature[torch.arange(0, batch_size), inds, :]
        n = feature[torch.arange(0, batch_size), 1 - inds, :]
        proto = torch.mean(top_feat_t, dim=0).view(1, -1).repeat(batch_size, 1)
        # loss = triplet_loss(proto, p, n)
        loss_ord = triplet_loss(top_feat_t, p, n)

        # compute accuracy
        d_a0a1 = F.pairwise_distance(top_feat_t, feature[:,0,:])
        d_a0a2 = F.pairwise_distance(top_feat_t, feature[:,1,:])
        sub_pix = iou[:,1] - iou[:,0]
        sub_d = d_a0a1 - d_a0a2
        ...
        if DEBUG:
            ...
            # for i in range(5):
            #     print(i, ' box ', boxes[i])
            print('iou rewards deltas ', torch.sign(iou[:5, 1:]-iou[:5,:-1]))
    ...
    acc_loc = 100. * correct_loc / count
    acc_inst = 100. * correct_ord_inst / count
    acc_r = 100. * correct_reward / (count*args.seq_len)
    acc_proto = 100. * correct_ord_proto / count
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if test_acc > best_acc:
                ...
                no_improve_epoch += 1
            if save_model:
                print('epoch ', epoch, 'saving model acc ',  best_acc)
                torch.save({
                    'epoch': epoch,

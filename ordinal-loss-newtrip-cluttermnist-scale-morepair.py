...
def init_dataloader():
    ...
    trainset = MNIST_CoLoc(root='.', train=True, digit=args.digit, sample_size=args.sample_size,
                           ...
                           target_transform=MyBoxScaleTransform(num=3))
...
def feed_data(model, data_loader, is_train):
    ...
    for batch_idx, (x0s, target, tis, ious) in enumerate(data_loader):
        batch_size = x0s.shape[0]
        x0s, target, tis, ious = x0s.to(device), \
                              target.to(device).float(), \
                              tis.to(device), ious.to(device)
...
if __name__ == "__main__":
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer,
                                                step_size=args.steps[0], gamma=0.1)

...
def feed_data(model, agent, data_loader, is_train, upload=True):
    ...
    pol_losses, ent_losses, cls_losses, rewards = [], [], [], []
    correct, correct_af = 0, 0
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        # cls loss
        loss_cls = triploss(support_top_feat_t, support_y[:, -1])[0]
        if phase == "Test":
            loss = loss_cls * args.lamb_cls
        else:
        ...
        if phase == 'Test': # localize after embedding update
            actor = Actor(target_x, min_box_side=28)
            if args.hidden_size:
                h_t = torch.zeros(
                    batch_size,
            ...
            pred_boxes_af = torch.tensor([[0, 0,
                                        83, 83]]).float().repeat(batch_size, 1).to(device)
            rois = torch.cat((torch.arange(0, batch_size).float().view(-1, 1).to(device),
                              pred_boxes_af), dim=1)
            with torch.no_grad():
            ...
            for t in range(args.seq_len):
                if args.hidden_size:
                    ...
                    logits, actions = agent(target_embed)
                state, pred_boxes_af = actor.takeaction(actions)
                rois = torch.cat((torch.arange(0, batch_size).float().view(-1, 1).to(device),
                                  pred_boxes.to(device)), dim=1)
                with torch.no_grad():
                    target_embed, target_top_feat = model.get_roi_embedding(rois)
        ...
        if phase == 'Test':
            # calculate accuracy
            iou = torch.diagonal(box_iou(pred_boxes_af.float(),
                                         target_y[:, :4]), 0)
            correct_af += (iou >= 0.5).sum()
    ...
    if phase == 'Test':
        acc_af = 100. * correct_af / total_img
    ...
    if phase == 'Train':
        ...
        log_message.update({'{} Loc AF Acc'.format(phase): acc_af})
    if upload:
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        #finetune_log = feed_data(net, agent, data_loader=test_loader, is_train=True)
        #with torch.no_grad():

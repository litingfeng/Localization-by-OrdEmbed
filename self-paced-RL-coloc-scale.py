...
def feed_data(model, agent, data_loader, is_train):
    ...
    for batch_idx, (x0s, target, ti, ioui) in enumerate(data_loader):
        batch_size = x0s.shape[0]
        x0s, target, ti, ioui = x0s.to(device),  target.to(device).float(), \
                                ti.to(device), ioui.to(device)
        ...
        actor = Actor(x0s, min_box_side=20)

...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (a1, a2, x0s, xis, xjs, target) in enumerate(train_loader):
            ...
            loss_ij = loss_i - loss_j
            sub_angle = a1 - a2
            mask = (torch.sign(loss_ij) == torch.sign(sub_angle))
            loss_order = torch.abs(loss_ij)
            loss_order[mask==True] = 0
            #F.relu(distance_positive - distance_negative + self.margin)
            correct_order += mask.sum().item()
            ...
            loss = loss_order.mean()
            #loss = loss_0.mean() + loss_order.mean() * 100
            #loss = loss_i.mean() + loss_j.mean() + loss_order.mean() * 50
            loss.backward()
            optimizer.step()
            # loss_order.backward()
            # optimizer2.step()
            ...
            losses_order.append(loss_order.mean().item())
        ...
        acc_order = 100. * correct_order / len(train_loader.dataset)
        accs.append(acc)
    ...
    acc_oder = 100. * correct_order / len(test_loader.dataset)

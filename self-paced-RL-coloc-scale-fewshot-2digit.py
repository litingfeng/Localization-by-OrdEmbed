...
def init_model():
    # using ae+ordinal
    net = Net_ae_proj(pooling_size=args.pooling_size, dim=args.dim,
...
def feed_data(model, agent, data_loader, is_train, upload=True):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            embed_t_n, top_feat_t_n = model(data, rois_t0_noise)
        ...
        if 'proto' in args.train_mode: #
            ...
            proto_n = top_feat_t_n.view(-1, args.samples_per_class, args.dim).mean(1)  # (num_group, 512)

...
def feed_data(model, data_loader, is_train, upload=True):
    ...
    for batch_idx, (data, target, boxes, ious) in enumerate(data_loader):
        ...
        recon_img, pooled_target_ord, pooled_target_cls = model(data, rois_targets) # (bs, 1, 84, 84), (2bs, 1600)
        pos_pooled, neg_pooled = pooled_target_ord.view(batch_size, 2, -1)[:, 0], \
                                 pooled_target_ord.view(batch_size, 2, -1)[:, 1]
        _, pooleds_ord, pooleds_cls = model(data, rois)  # (bs, 1, 84, 84), (4bs, 1600)
        pooleds_ord = pooleds_ord.view(batch_size, 4, -1)

        loss_recon = criterion(recon_img, data)

        '''compute loss order'''
        pos_f = pooleds_ord[:, :2] # (bs, 2, dim)
        ...
        loss_cls = contrast_loss(pooled_target_cls, target.reshape(-1, 5)[:, -1])
        ...
        d_a0a1 = F.pairwise_distance(pos_pooled, pooleds_ord[:, 0])
        d_a0a2 = F.pairwise_distance(pos_pooled, pooleds_ord[:, 1])
        ...
        d_a0a1 = F.pairwise_distance(neg_pooled, pooleds_ord[:, 2])
        d_a0a2 = F.pairwise_distance(neg_pooled, pooleds_ord[:, 3])
        sub_pix_bg = neg_ious[:, 1] - neg_ious[:, 0]
        sub_d = d_a0a1 - d_a0a2
        mask = (torch.sign(sub_d) == torch.sign(sub_pix_bg))
        correct_inst_bg += mask.sum().item()

        d_a0a1 = F.pairwise_distance(pos_proto, pooleds_ord[:, 0])
        d_a0a2 = F.pairwise_distance(pos_proto, pooleds_ord[:, 1])
        sub_d = d_a0a1 - d_a0a2
        mask = (torch.sign(sub_d) == torch.sign(sub_pix))
        correct_proto_fg += mask.sum().item()

        d_a0a1 = F.pairwise_distance(neg_proto, pooleds_ord[:, 2])
        d_a0a2 = F.pairwise_distance(neg_proto, pooleds_ord[:, 3])
        sub_d = d_a0a1 - d_a0a2
        mask = (torch.sign(sub_d) == torch.sign(sub_pix_bg))
        correct_proto_bg += mask.sum().item()

         # acc other
        pooled_i = pooleds_ord[:, 0].view(-1, args.samples_per_class, args.dim)[rnd_inds].view(-1, args.dim)
        pooled_j = pooleds_ord[:, 1].view(-1, args.samples_per_class, args.dim)[rnd_inds].view(-1, args.dim)
...
def evaluate_siam(model, data_loader):
    ...
    for batch_idx, (data, target, boxes, ious) in enumerate(data_loader):
        ...
        _, pooled_target_ord, pooled_target_cls = model(data, rois_targets)  # (2bs, dim)
        pos_pooled, neg_pooled = pooled_target_cls.view(batch_size, 2, -1)[:, 0], \
                                 pooled_target_cls.view(batch_size, 2, -1)[:, 1]
...
if __name__ == '__main__':
    ...
    model = AutoencoderProj_cls(channel=1, pooling_size=args.pooling_size, dim=args.dim,

...
from datasets.clutter_mnist_scale_anchor_2digit import MNIST_CoLoc
...
from losses.selectors import AllPositivePairSelector
...
def feed_data(model, data_loader, is_train, upload=True):
    ...
    for batch_idx, (data, target, boxes, ious) in enumerate(data_loader):
        ...
        #                     target[:,0].squeeze()), dim=1)
        # rois_noise = torch.cat((torch.arange(0, batch_size).float().view(-1, 1).to(device),
        #                     target[:, 1].squeeze()), dim=1)
        rois_targets = torch.cat((torch.arange(0, batch_size).unsqueeze(1).repeat(1,2).view(-1, 1).to(device),
                             target.view(-1, 5)[:, :4]), dim=1)
        rois = torch.cat((torch.arange(0, batch_size).unsqueeze(1).repeat(1,2).view(-1, 1).to(device),
                          boxes.view(-1, 4)), dim=1) # (2bs, 5)
        ...
        _, pooleds = model(data, rois)  # (bs, 1, 84, 84), (2bs, 1600)
        pooleds = pooleds.view(batch_size, 2, -1)
        ...
        proto_p = pos_pooled.view(-1, args.samples_per_class, 512).mean(1) # (num_group, 512)
        proto_p = proto_p.unsqueeze(1).repeat(1, args.samples_per_class, 1).view(-1, 512)
        proto_n = neg_pooled.view(-1, args.samples_per_class, 512).mean(1)  # (num_group, 512)
        proto_n = proto_n.unsqueeze(1).repeat(1, args.samples_per_class, 1).view(-1, 512)
        loss_ord = triplet_loss(proto_p, p, n)
        ...
        sub_pix = ious[:, 1] - ious[:, 0]
        sub_d = d_a0a1 - d_a0a2
        mask = (torch.sign(sub_d) == torch.sign(sub_pix))
        correct_inst += mask.sum().item()

        d_a0a1 = F.pairwise_distance(proto_p, pooleds[:, 0])
        d_a0a2 = F.pairwise_distance(proto_p, pooleds[:, 1])
        sub_d = d_a0a1 - d_a0a2
        mask = (torch.sign(sub_d) == torch.sign(sub_pix))
        correct_proto += mask.sum().item()
         # acc other
        pooled_i = pooleds[:, 0].view(-1, args.samples_per_class, 512)[rnd_inds].view(-1, 512)
        pooled_j = pooleds[:, 1].view(-1, args.samples_per_class, 512)[rnd_inds].view(-1, 512)
        ioui = ious[:, 0].view(-1, args.samples_per_class)[rnd_inds].view(-1)
        iouj = ious[:, 1].view(-1, args.samples_per_class)[rnd_inds].view(-1)
        ...
        d_a0a1 = F.pairwise_distance(proto_p, pooled_i)
        d_a0a2 = F.pairwise_distance(proto_p, pooled_j)
        sub_d = d_a0a1 - d_a0a2
        mask = (torch.sign(sub_d) == torch.sign(sub_pix))
...
def evaluate_siam(model, data_loader):
    ...
    print(("=> loaded model checkpoint epoch {} {}\n\tacc_inst {:.2f}"
           ...
           "\tacc_proto_other {:.2f}".format(epoch, 'last', acc_inst, acc_proto,
    ...
    for batch_idx, (data, target, boxes, ious) in enumerate(data_loader):
        ...
        pos_pooled, neg_pooled = pooled_target.view(batch_size, 2, -1)[:, 0], \
                                 pooled_target.view(batch_size, 2, -1)[:, 1]
        proto = pos_pooled.view(-1, args.samples_per_class, 512).mean(1)  # (num_group, 512)
        proto_n = neg_pooled.view(-1, args.samples_per_class, 512).mean(1)  # (num_group, 512)
        ...
        correct_fg += (pos_dist < -60).sum()
        correct_bg += (neg_dist < -60).sum()
...
if __name__ == '__main__':
    ...
    contrast_loss = OnlineContrastiveLoss(args.margin2, AllPositivePairSelector())

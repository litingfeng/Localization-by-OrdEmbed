...
def feed_data(model, agent, data_loader, is_train= True, upload=True):
    ...
    for batch_idx, (data_org, target_org, _) in enumerate(data_loader):
        ...
        for t in range(args.classes_per_set): # for each task (class)
            data, target = data_org[t*args.samples_per_class: (t+1)*args.samples_per_class], \
                           target_org[t * args.samples_per_class: (t + 1) * args.samples_per_class]
            ...
            set_loss, q, loss_ent, rewards_deltas, iou = localize(model, agent, data, target) # compute loss using updated model

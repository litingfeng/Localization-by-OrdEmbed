...
def train(policy_net, target_net, embedding_net, data_loader):
    ...
    total_pol_loss, total_rewards,ord_losses = [], [], []
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        org_embed_t = embedding_net(data, rois_t0)
        ...
        feature = embedding_net(data, rois.detach()).view(bs, 2, -1)  # (bs, 2, dim)
...
def evaluate(policy_net, embedding_net, data_loader):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        rois = torch.cat((inds.view(-1, 1),
                          smp_boxes.view(-1, 4)), dim=1)  # (num*2, 5)
        feature = embedding_net(data, rois.detach()).view(bs, 2, -1)  # (bs, 2, 1024)

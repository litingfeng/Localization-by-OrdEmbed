...
from util.utils import box_iou, RunningMeanStd
...
def feed_data(model, agent, rnd, data_loader, is_train):
    ...
    for batch_idx, (x0s, target) in enumerate(data_loader):
        ...
        next_obs = np.zeros((args.seq_len, batch_size, 512))
        ...
        for t in range(args.seq_len):
            ...
            predict_next_state_feature, target_next_state_feature = rnd(embed)
            intrinsic_reward = rnd.compute_intrinsic_reward(
                ((embed.cpu().detach().numpy() - obs_rms.mean)
                 / np.sqrt(obs_rms.var)).clip(-5, 5)) # (bs,)

            action_seq[:, t] = actions.view(1, -1)
            logits_seq.append(logits)
            boxes.append(pred_boxes)
            int_reward[:, t] = intrinsic_reward.view(1, -1) # reverse???
            next_obs[t] = embed.cpu().detach().numpy()
        ...
        gamma_ltri = _build_discounts_matrix(args.seq_len, args.gamma).cuda()
        total_reward_per_env = torch.mm(int_reward, gamma_ltri).permute(1, 0)
        mean, std, count = torch.mean(total_reward_per_env).item(), \
                           torch.std(total_reward_per_env).item(), len(total_reward_per_env)
        reward_rms.update_from_moments(mean, std ** 2, count)

        # normalize intrinsic reward
        int_reward /= np.sqrt(reward_rms.var)
        ...
        next_obs = next_obs.reshape(-1, 512)
...
if __name__ == "__main__":
    ...
    # normalize obs, using embedding from net
    print('Start to initailize observation normalization parameter.....')
    next_obs = []
    pre_obs_norm_step = 5
    # num_step = args.seq_len
    # num_worker = args.batch_size
    obs_rms = RunningMeanStd(shape=(1, 512))
    for e in range(pre_obs_norm_step):
        for batch_idx, (data, target) in enumerate(train_loader):
            data, target = data.to(device), target.to(device)
            batch_size = data.shape[0]
            actor = Actor(data, min_box_side=28)  # TODO
            for t in range(args.seq_len):
                actions = np.random.randint(0, args.num_act, size=(batch_size,))
                state, pred_boxes = actor.takeaction(actions)
                rois = torch.cat((torch.arange(0, batch_size).float().view(-1, 1).to(device),
                                  pred_boxes.to(device)), dim=1)
                with torch.no_grad():
                    if t == 0:
                        ...
                        embed = net.get_roi_embedding(rois)
                next_obs.append(embed.cpu().numpy())

            next_obs = np.stack(next_obs)  # (seq_len, bs, dim)
            obs_rms.update(next_obs.reshape(-1, 512))
            next_obs = []
    print('End to initalize...')
    reward_rms = RunningMeanStd()  # runnning mean of intrinsic reward
    discounted_reward = RewardForwardFilter(gamma=0.99)

...
class myMNIST(datasets.MNIST):
    def __init__(self, root, subset, datapath=None, train=True, transform=None,
        ...
        if not datapath:
            ...
            self.newdata, self.newtargets = np.array(self.newdata)[inds[:50]], \
                                            np.array(self.newtargets)[inds[:50]]
            self.rot_inds = np.random.choice(50, size=25, replace=False) # select 100 to rotate
        ...
        if self.subset == 'original_all': # use all original(non-roated) 200 samples
            ...
            self.data = [self.data[i] for i in self.newdata]
            self.data = [ torch.from_numpy(np.rot90(data.numpy()).copy())
...
if __name__ == "__main__":
    ...
    dataset = myMNIST(root = '.', subset = mode,
                      datapath='.',
                      train=True, transform=transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.1307,), (0.3081,))
        ]))
    test_set = myMNIST(root='.', subset='original_all',
                      datapath='.',
    ...
    run_accs = {'train': [], 'test': []}
    for run in range(10):
        embedding_net = EmbeddingNet_Simple()
        ...
        testing
        '''
        ckpt = torch.load('{}.pth.tar'.format(mode))

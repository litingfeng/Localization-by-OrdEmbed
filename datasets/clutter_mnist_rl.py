...
if __name__ == '__main__':
    ...
    #         np.save('{}/{}_moreclutter_center_data'.format(savedir, digit), dataset.new_digit_data)
    #         np.save('{}/{}_moreclutter_center_label'.format(savedir, digit), dataset.new_targets)
    #         # # save each image
    #         # for i in range(dataset.new_digit_data.shape[0]):
    #         #     img = dataset.new_digit_data[i]
    #         #     img = Image.fromarray(img).convert("L")
    #         #     img.save('{}/{}_{}.png'.format(savedir, digit, i))

    dataset = MNIST_CoLoc(root='.', clutter=True, train=True, digit=4,
                          transform=Compose([Resize(84)]),
                          datapath=savepath)

...
class CUB_fewshot():
    def __init__(self, root, img_size, anchors=None,
...
if __name__ =='__main__':
    ...
    dataset = CUB_fewshot('/research/cbim/vast/tl601/Dataset/CUB_200_2011',
                          img_size=84,mode='base', anchors=anchors,
                          transform=t.Compose([
                              t.ConvertFromPIL(),
                              t.ToPercentCoords(),
                              t.Resize(84),
    ...
    train_loader = torch.utils.data.DataLoader(
        dataset, batch_size=2, shuffle=False, **kwargs)
    ...
    for i, (data, target, boxes, ious) in enumerate(train_loader):
        print('data ', data.shape, ' target ', target.shape, ' boxes ', boxes.shape, ' ious ', ious.shape)
        exit()
        iouis.append(iou1)
        ioujs.append(iou2)
        w, h = target[:, 2] - target[:, 0], target[:, -2] - target[:, 1]
        ar = h / w
        ars.append(ar)
        ws.append(w)
        hs.append(h)

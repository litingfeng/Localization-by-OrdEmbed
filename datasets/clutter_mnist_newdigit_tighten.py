...
class MNIST_CoLoc(datasets.MNIST):
    def __init__(self, root, digit, train=True, transform=None,
        ...
        for i in range(self.new_digit_data.shape[0]):
            ...
            gt_bbox = self.create_gt_bbox(self.new_digit_data[i, y1:y2, x1:x2], 20)
            gt_bbox[0] += x1
            gt_bbox[1] += y1
        ...
        if train:
            ...
            for i in range(self.supp_data_array.shape[0]):
                img, target = self.supp_data_array[i], self.supp_targets[i]
...
if __name__ == '__main__':
    ...
    trainset = MNIST_CoLoc(root='.', train=True, digit=5, support_size=5,

...
class OmniglotDataset(data.Dataset):
    ...
    splits_folder = os.path.join('splits', 'Tingfeng5_small2')
    raw_folder = 'raw'
    ...
    def __init__(self, root, mode='train', transform=None,
        ...
        self.new_x, self.y_box = self._generate_patch_img() # new_x (num, 84, 84), y_box (num, 2, 4)
    ...
    def _generate_patch_img(self, ):
        ...
        y_box = torch.zeros((len(self.x), 2, 4), dtype=torch.float32)
        for i, data in enumerate(self.x):
            ...
            y_box[i, 0] = torch.tensor([x_d, y_d, x_d + 28 - 1, y_d + 28 - 1])

            # sample second char
            cls = np.random.randint(self.num_cls)
            ind = np.random.randint(len(self.cls2inds[cls]))
            noise_x, noise_y = self.x[ind], self.label[ind]
            ...
            y_box[i, 1] = torch.tensor([x_d2, y_d2, x_d2 + 28 - 1, y_d2 + 28 - 1])
    ...
    def __getitem__(self, idx):
        x, target, target_box = self.new_x[idx], self.label[idx], self.y_box[idx]
        target = np.hstack((target_box[0], np.array(target)))

...
if __name__ == '__main__':
    ...
    # view last epoch
    x = torch.cat(losses_ij[-1])
    f = torch.cat(feat_ijs[-1])
    y = torch.cat(save_ts[-1]).cpu()
    round_y = torch.round(y[:, 0]*10**3)/ 10**3
    classes = {v: k for k,v in
               enumerate(sorted(set(list(round_y.numpy()))))}
    ...
    with torch.no_grad():
        ...
        dataset = MyDataset(data.view(20000,1, 1, 1), target,

...
def feed_data(model, data_loader, is_train):
    ...
    train_losses, losses_0, losses_i, losses_order = \
        [], [], [], []
    correct_0, correct_i, correct_order,\
    correct_order_sub, correct_order_sub_l = 0, 0, 0, 0, 0
    ...
    for batch_idx, (ai, aj, x0s,
        ...
        feature_0, output_0 = net(x0s)  # output: (bs, n_classes)
        ...
        logits_0 = torch.gather(output_0, 1, target.unsqueeze(1))
        logits_i = torch.gather(output_i, 1, target.unsqueeze(1))
        logits_j = torch.gather(output_j, 1, target.unsqueeze(1))
        l = torch.stack((logits_i, logits_j), 1)  # (bs, 2, 1)

        inds = torch.min(a, 1).indices
        p = l[torch.arange(0, batch_size), inds]
        n = l[torch.arange(0, batch_size), 1 - inds]
        loss_order = triplet_loss(logits_0, p, n)

        # a = torch.stack((ai, aj), 1)  # (bs, 2)
        # logits_0 = output_0.max(1, keepdim=True)[0] # (bs, 1)
        # logits_i = output_i.max(1, keepdim=True)[0]
        # logits_j = output_j.max(1, keepdim=True)[0]
        # l = torch.stack((logits_i, logits_j), 1)  # (bs, 2, 1)
        #
        # inds = torch.min(a, 1).indices
        # p = l[torch.arange(0, batch_size), inds]
        ...
        # update model
        loss = loss_0.mean() + loss_order * args.lamb
        if phase == 'Train':
            loss.backward()
            optimizer.step()
        ...
        correct_order_sub_l += mask.sum().item()

        logits_ij = logits_i.view(-1) - logits_j.view(-1)
        sub_angle = ai - aj
        mask = (torch.sign(logits_ij) == torch.sign(sub_angle))
        correct_order_sub += mask.sum().item()

        d_a0a1 = F.pairwise_distance(logits_0, logits_i)
        d_a0a2 = F.pairwise_distance(logits_0, logits_j)
    ...
    acc_sub_l = 100. * correct_order_sub_l / len(data_loader.dataset)
    ...
    log_message = {"{} Total Loss".format(phase): total_loss,
                   ...
                   '{} Ord Acc_sub_l'.format(phase): acc_sub_l,
                   }
    if phase == 'Train':

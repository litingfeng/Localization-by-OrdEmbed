"""
four training mode: self/ support proto/, fewshot setting
...
def feed_data(model, data_loader, is_train, upload=True):
    ...
    total_losses, ord_losses, cls_losses = [], [], []
    ...
    for batch_idx, (data, target, boxes, ious) in enumerate(data_loader):
        ...
        '''separate to support & target set'''
        target_data = data.view(args.classes_per_set, args.samples_per_class * 2,
                    3, args.img_size, args.img_size)[:, args.samples_per_class:, :, :, :] # (N,K,1,84,84)
        ...
        _, pooled_gt_all, cls_out_all  = model(data, rois_targets) # (bs, 1, 84, 84), (bs, dim)
        _, pooleds, _ = model(target_data.reshape(-1, 3, args.img_size, args.img_size), rois) # (N*K*2, dim)
        ...
        proto = proto.unsqueeze(1).repeat(1, args.samples_per_class, 1).view(-1, args.dim)  # (N*K, dim)

        if args.train_mode == 'self':
            loss_ord = triplet_loss(pooled_gt_tar.reshape(args.classes_per_set*args.samples_per_class, args.dim),
                                p, n) #(bs,)
        elif args.train_mode == 'proto':
            loss_ord = triplet_loss(proto, p, n)  # (bs,)

        '''compute siamese loss for class using support set data & gt'''
        target = target.view(args.classes_per_set, args.samples_per_class * 2, 5)  # (N, 2K, 5)
        support_y = target[:, :args.samples_per_class, :]
        # loss_cls = triploss2(pooled_gt_supp.reshape(args.classes_per_set*args.samples_per_class, -1),
        #                     support_y.reshape(-1, 5)[:, -1])[0]

        '''classification'''
        loss_cls = criterion(cls_out_all, target_org[:, -1].long())

        loss = loss_ord * args.lamb + loss_cls * args.lamb_cls
...
if __name__ == '__main__':
    ...
    model = Net(pooling_size=args.pooling_size, pretrained=True, base=args.net, fixed=args.freeze, dim=args.dim,
                num_classes=args.num_cls, pooling_mode=args.pooling_mode).to(device)
    wandb.watch(model, log_freq=10)
    wandb.config.update(args)

    triplet_loss = nn.TripletMarginLoss(margin=args.margin, p=2)
    triploss2 = OnlineTripletLoss(args.margin_cls,
                                  SemihardNegativeTripletSelector(args.margin_cls))
                                 #AllTripletSelector())

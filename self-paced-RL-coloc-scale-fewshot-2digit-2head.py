...
from datasets.clutter_mnist_scale_rl_2digit import MNIST_CoLoc
from models.mnist_ae_ord import AutoencoderProj_cls
...
def init_model():
    # using ae+ordinal
    net = AutoencoderProj_cls(pooling_size=args.pooling_size, dim=args.dim,
...
def feed_data(model, agent, data_loader, is_train, upload=True):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        rois_t0 = torch.cat((torch.arange(0, batch_size).float().view(-1, 1),
                            target[:, 0, :4]), dim=1).to(device)
        rois_t0_noise = torch.cat((torch.arange(0, batch_size).float().view(-1, 1),
                             target[:, 1, :4]), dim=1).to(device)
        ...
        with torch.no_grad():
            embed_t, top_feat_t_ord, top_feat_t_cls = model(data, rois_t0)
            embed_t_n, top_feat_t_n_ord, top_feat_t_n_cls = model(data, rois_t0_noise)
            embed, top_feat_ord, top_feat_cls = model.get_roi_embedding(rois)
        top_feat_t = torch.cat((top_feat_t_ord, top_feat_t_n_cls), 1)
        top_feat_t_n = torch.cat((top_feat_t_n_ord, top_feat_t_n_cls), 1)
        # rewards_all[:, 0] = F.pairwise_distance(embed_t.view(embed_t.shape[0], -1),
        #                                         embed.view(embed.shape[0], -1))
        if 'proto' in args.train_mode: #
            proto = top_feat_t.view(-1, args.samples_per_class, args.dim * 2).mean(1)  # (num_group, 2*512)
            proto_n = top_feat_t_n.view(-1, args.samples_per_class, args.dim * 2).mean(1)  # (num_group, 2*512)
            if args.train_mode == 'shuffle_proto': #shuffle proto or top top_feat?
                proto = proto[rnd_inds]
                proto_n = proto_n[rnd_inds]
                # rewards_all[:, 0] = F.pairwise_distance(proto, top_feat.view(-1,
                #                        args.samples_per_class, 512)[rnd_inds].view(-1,512))
            proto = proto.unsqueeze(1).repeat(1, args.samples_per_class, 1).view(-1, args.dim * 2)
            proto_n = proto_n.unsqueeze(1).repeat(1, args.samples_per_class, 1).view(-1, args.dim * 2)
            rewards_all[:, 0] = F.pairwise_distance(proto,
                                                    torch.cat((top_feat_ord, top_feat_cls), 1))
            # rewards_all[:, 0] = (F.pairwise_distance(proto, top_feat) -
            #                     F.pairwise_distance(proto_n, top_feat)) * 1.

        elif args.train_mode == 'shuffle_self':
            top_feat_t = top_feat_t.view(-1, args.samples_per_class,
                                         args.dim*2)[rnd_inds].view(-1,args.dim * 2)
            rewards_all[:, 0] = F.pairwise_distance(top_feat_t, torch.cat((top_feat_ord, top_feat_cls), 1))
        ...
        for t in range(args.seq_len):
            ...
            if 'proto' in args.train_mode:  #
                rewards_all[:, t + 1] = F.pairwise_distance(proto, torch.cat((top_feat_ord, top_feat_cls), 1))
                # rewards_all[:, t+1] = (F.pairwise_distance(proto, top_feat) -
                #                     F.pairwise_distance(proto_n, top_feat)) * 1.
            elif args.train_mode == 'shuffle_self':
                rewards_all[:, t + 1] = F.pairwise_distance(top_feat_t, torch.cat((top_feat_ord, top_feat_cls), 1))

            # # go to bg digit, negative reward
            # neg_iou = torch.diagonal(box_iou(pred_boxes.float(),
            #                                  target[:, 1, :4]), 0)
            # neg_rewards = torch.zeros(batch_size).to(device)
            # wrong = neg_iou >= 0.5
            # neg_rewards[wrong] = 4 * F.pairwise_distance(proto, top_feat)[wrong]
            # rewards_all[:, t + 1] += neg_rewards

...
class Actor:
    def __init__(self, images, targets, min_box_side=50, sign=1):
        self.images = images
    ...
    def _initialize_iou(self):
        for i, agent_window  in enumerate(self.agent_windows):
            self.ious[i] = self.intersectionOverUnion(agent_window, self.targets[i])
    ...
    def ReturnReward(self, new_iou, old_iou, termination):
        ...
        """
        reward = 0.
        reward = np.sign(new_iou - old_iou) if self.sign == 1 else new_iou - old_iou

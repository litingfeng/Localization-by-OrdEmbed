...
class Actor:
    def __init__(self, images, target, min_box_side=50):
        self.images = images
        self.target = target
        self.iou = 0
        self.min_box_side = min_box_side
        self.image_w, self.image_h = self.images.shape[3], self.images.shape[2]
        self.agent_window = np.array([0., 0., self.image_w-1, self.image_h-1])
    ...
    def ComputingReward(self, agent_window, termination=False):
        ...
        new_iou = self.intersectionOverUnion(agent_window, self.target)

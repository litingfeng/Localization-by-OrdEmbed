...
class BoxSampler(object):
    ...
    def sample(self, inputBoxSet, imgSize):
        ...
        validIndices = torch.ByteTensor(boxNumber).fill_(1)
        flag = 0
        ...
        sampledBoxSet = torch.FloatTensor(positiveRoI_number, 4).fill_(-1)
        gt_inds = torch.LongTensor(positiveRoI_number).fill_(0)
        ...
        return sampledBoxSet, inputBoxSetExtended[:, -1].type(torch.LongTensor), generated_box_overlaps, gt_inds
    ...
    def InstanceAllocation(self, inputBoxSet):
        ...
        classIndices = torch.FloatTensor(classNumber, inputBoxSet.size()[0]).fill_(0)
        ...
        extendedInputBoxSet = torch.FloatTensor(positiveRoI_number, 5).fill_(0)
        instanceNumber = inputBoxSet.size()[0]

        indexTracker = 0
        perInputAllocation = torch.FloatTensor(inputBoxSet.size()[0]).fill_(0)
    ...
    def IoUAllocation(self, inputBoxSet, positiveRoI_number):
        ...
        IoUSet = (self.IoU_bin_bases[IoUIndices] + torch.rand(positiveRoI_number) * self.bin_width[IoUIndices])
    ...
    def findBottomRightBorders(self, inputBox, IoU, boxArea, proposedx1, proposedy1, limitLeftX, limitRightX, limitTopY,
        ...
        y2TR = torch.arange(limitTopY, inputBox[3] + self.IoU_limit_precision, step=self.IoU_limit_precision)
        ...
        x2BR = torch.arange(limitRightX, inputBox[2] - self.IoU_limit_precision, step=-self.IoU_limit_precision)
        y2BR = (I / IoU - boxArea + I) / (x2BR - proposedx1)
        y2BR += proposedy1

        y2BL = torch.arange(limitBottomY, inputBox[3] - self.IoU_limit_precision, step=-self.IoU_limit_precision)
        ...
        x2TL = torch.arange(limitLeftX, inputBox[2] + self.IoU_limit_precision, step=self.IoU_limit_precision)
    ...
    def BoundingBoxGenerator(self, inputBox, IoUSet, numBoxes):
        sampledBox = torch.FloatTensor(numBoxes, 4).fill_(-1)
        ...
        for i in range(numBoxes):
            ...
            if np.random.uniform() < 0.5:
                flag = 1
                inputBox = torch.tensor([1 - box[2], 1 - box[3], 1 - box[0], 1 - box[1], box[4]])
            else:
                flag = 0
                inputBox = box
            ...
            if flag == 1:
                sampledBox[i, :] = torch.tensor(
                    [1 - sampledBox[i, 2], 1 - sampledBox[i, 3], 1 - sampledBox[i, 0], 1 - sampledBox[i, 1]])

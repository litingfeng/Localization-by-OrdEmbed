...
def init_dataloader():
    ...
    trainset = MNIST_CoLoc(root='.', train=True, digit=args.digit, bg_name=args.bg_name,
...
def init_model():
    embed_net = Net_ae_proj(pooling_size=args.pooling_size,
...
def feed_data(embed_net, agent, data_loader, is_train, upload=True):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        if use_top_feat:
            # dists = F.pairwise_distance(supp_top_feat.unsqueeze(0).repeat(
            #     batch_size, 1, 1).view(-1, 512), top_feat.unsqueeze(1).repeat(1,
            #                 args.support_size, 1).view(-1, 512))
            #
            # rewards_all[:, 0] = dists.view(batch_size, -1).min(1).values
            rewards_all[:, 0] = F.pairwise_distance(top_feat_t, top_feat)
            #rewards_all[:, 0] = F.pairwise_distance(proto_feat, top_feat)
        else:
            # embed_ = embed.view(embed.shape[0], -1)
            # dists = F.pairwise_distance(supp_embed.unsqueeze(0).repeat(
            #     batch_size, 1, 1).view(-1, 64*args.pooling_size*args.pooling_size),
            #     embed_.unsqueeze(1).repeat(1, args.support_size, 1).view(-1,
            #         64*args.pooling_size*args.pooling_size))
            #
            # rewards_all[:, 0] = dists.view(batch_size, -1).min(1).values
        ...
        for t in range(args.seq_len):
            ...
            if use_top_feat:
                ...
                #                 args.support_size, 1).view(-1, 512))
                #
                # rewards_all[:, t] = dists.view(batch_size, -1).min(1).values
                # if t == 9:
                #     print(batch_idx, ' ', t, ' inds ', dists.view(batch_size, -1)[:5], '\n',
                #           dists.view(batch_size, -1).min(1)[1][:5], '\t',
                #           dists.view(batch_size, -1).min(1)[0][:5])
            else:
                # embed_ = embed.view(embed.shape[0], -1)
                # dists = F.pairwise_distance(supp_embed.unsqueeze(0).repeat(
                #     batch_size, 1, 1).view(-1, 64 * args.pooling_size * args.pooling_size),
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        train_log = feed_data(embed_net, agent, data_loader=supp_loader, is_train=True)
        scheduler_ag.step()
